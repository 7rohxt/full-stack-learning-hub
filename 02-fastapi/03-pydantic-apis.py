...
class TodoBase(BaseModel):  # why no id? --> this is for request model. so id will be autocalculated. (client will not request with ID)              
...
class Todo(TodoBase):    # This will be used for response model (for response, we need id, name, description and priority)
...
@app.get('/todo/{todo_id}', response_model=Todo) # path params
...
def get_all_todos(first_n: Optional[int] = None):
    if first_n:
